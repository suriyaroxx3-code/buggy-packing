// billing.create.jsx — Create Invoice, fully static localStorage-backed
...
function genInvoiceNo() {
  const existing = billingStore.getAll().map((r) => r.id);
  let n = 184;
  let id;
...
function Page() {
  ...
  const saveBill = (status) => {
    ...
    const bill = {
      ...
      place:      placeOfSupply.trim(),
      items,
    };

    billingStore.add(bill);
              ...
              {items.map((it, idx) => (
                <tr key={idx} className="border-b border-border/50 last:border-0">
                  ...
                  <td className="px-4 sm:px-6 py-2 text-right">
                    <button onClick={() => removeItem(idx)} className="text-muted-foreground hover:text-destructive p-1 transition" title="Remove item">
